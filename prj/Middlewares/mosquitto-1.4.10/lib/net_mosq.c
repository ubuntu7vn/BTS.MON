...
int _mosquitto_try_connect(struct mosquitto *mosq, const char *host, uint16_t port, mosq_sock_t *sock, const char *bind_address, bool blocking)
{
	...
	*sock = INVALID_SOCKET;
	...
	memset(&ainfo, 0, sizeof(struct sockaddr_in));
	ainfo.sin_len = sizeof(struct sockaddr_in);
	ainfo.sin_family = AF_INET;
	ainfo.sin_port = PP_HTONS(port);
	ainfo.sin_addr.s_addr = inet_addr(host);

	*sock = socket(ainfo.sin_family, SOCK_STREAM, 0);
	if(*sock == INVALID_SOCKET) return MOSQ_ERR_EAI;
	...
	rc = connect(*sock, (struct sockaddr*)&ainfo, sizeof(ainfo));
#ifdef WIN32
		errno = WSAGetLastError();
#endif
	if(rc == 0 || errno == EINPROGRESS || errno == COMPAT_EWOULDBLOCK){
